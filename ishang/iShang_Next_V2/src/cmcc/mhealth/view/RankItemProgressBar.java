 ...
 * 
 */
public class RankItemProgressBar extends ProgressBar {
	private Drawable mIndicator;
	private int mOffset = 5;
	private TextPaint mTextPaint;
	private Formatter mFormatter;
	// TODO
	private static int mProgressDistanceNum = 5500;
	private static int mProgressDistanceX = 40;
	private int mProgressMax = -1;

	public RankItemProgressBar(Context context) {
		this(context, null);
	}

	public RankItemProgressBar(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public RankItemProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		if (a != null) {
			mTextPaint.setTextSize(a.getDimension(R.styleable.SaundProgressBar_textSize, 10));
			mTextPaint.setColor(a.getColor(R.styleable.SaundProgressBar_textColor, Color.WHITE));
	 ...
	 * implements the {@linkplain RankItemProgressBar.Formatter} interface.
	 * 
	 * @param formatter
	 */
	...
	protected synchronized void onDraw(Canvas canvas) {
		...
		if (mIndicator != null) {
			if (progressDrawable != null && progressDrawable instanceof LayerDrawable) {
				...
				// bounds
				progressDrawable.getBounds().top = mIndicator.getIntrinsicHeight();
		...
		if (mIndicator != null) {
			...
			String progressNum;
			if (mFormatter != null)
				progressNum = mFormatter.getText(getProgress());
			else
				progressNum = getProgress() - mProgressDistanceNum + "";
			if (mProgressMax != -1) {
				progressNum = mProgressMax + "";
			}
			canvas.drawText(progressNum, getIndicatorWidth() / 2 - mProgressDistanceX, getIndicatorHeight() / 2, mTextPaint);
	...
	public synchronized void setProgress(int progress) {
		if (progress + mProgressDistanceNum > getMax()) {
			mProgressMax = progress;
		} else {
			mProgressMax = -1;
		}
		super.setProgress(progress + mProgressDistanceNum);
